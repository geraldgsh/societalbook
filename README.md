...
<%= form_with(model: @post, local: true) do |form| %>
	...
	<div>
		<%= form.submit "Submit", class: "button is-dark is-fullwidth" %>
	</div>
	<br>
	<span class="picture">
    <%= form.file_field :picture, accept: 'image/jpeg,image/gif,image/png' %>
...
<nav class="navbar" role="navigation" aria-label="main navigation">
  ...
  <div id="navbarBasicExample" class="navbar-menu">
    <div class="navbar-start">
      ...
      <%= link_to 'Micropost', microposts_path, class: "navbar-item"%>
      <% if user_signed_in? %>
        <%= link_to 'Friends List', users_friends_path(current_user), class: "navbar-item"%>
      <%end%>

      <%= link_to 'Users',users_path, class: "navbar-item"%>

      <% if user_signed_in? %>
        <a class="navbar-item" href="<%=user_friend_requests_path%>">
          <p>Notifications</p>
          &nbsp;<span class="tag is-danger is-rounded"> <%=current_user.friend_requests.count%></span>
        </a>
      <%end%>
...
```erb
# app/views/users/friend_requests.html.erb
   ...
   <div class="row column is-one-third">
    <div class="card has-background-light">
      ...
      <div class="card-content">
        ...
        </div>friend.id, current_user.id

        <div class="contfreshent">
      ...
      <footer>
        <%= link_to "Accept Request", accept_friend_request_path(user.id, current_user.id), method: :put, class:"button  is-link is-fullwidth" %>
        <%= link_to "Cancel request", delete_friend_request_path(user.id, current_user.id), method: :delete, class:"button  is-danger is-link is-fullwidth" %>
      </footer>
    </div>
    </div>
    <% end%>
    ...
    <div class="card has-background-light">
      ...
      <div class="card-content">
        <div class="media">
          ...
          <div class="media-content">
            <p class="title is-4"><%= friend.name %></p>
            <p class="subtitle is-6"><%= friend.email %></p>
          </div>
        </div>
      ...
      <footer>
         <%= link_to "Unfriend :(", delete_friend_request_path(friend.id, current_user.id), method: :delete, class:"button  is-danger is-link is-fullwidth" %>
      </footer>
    </div>
    </div>
    <% end%>
